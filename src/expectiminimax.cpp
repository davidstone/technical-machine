...
std::vector<RankedMove> create_ranked_move(Team const & team);
...
std::vector<RankedMove> create_ranked_move(Team const & team) {
	// Have to think about how to do this properly
	std::vector<RankedMove> r;
	for (uint8_t index = 0; index != team.pokemon().move.size(); ++index)
