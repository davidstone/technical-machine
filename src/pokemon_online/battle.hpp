...
class Battle : public GenericBattle {
	private:
		...
		int16_t damage;
	public:
		Battle (std::random_device::result_type seed, std::string const & opponent_name, int const battle_depth, std::string const & team_file_name);
		Battle (std::random_device::result_type seed, std::string const & opponent_name, int const battle_depth, Team const & team);
