...
void attack_tests() {
	...
	set_stat_ev(pokemon, RegularStat::def, IV(31_bi), EV(EV::max));
...
void special_attack_tests() {
	...
	set_stat_ev(pokemon, RegularStat::spa, IV(31_bi), EV(EV::max));
...
void min_defense_test() {
	...
	auto pokemon = defender.pokemon();
	pokemon.switch_in(generation, weather);
	set_stat_ev(pokemon, RegularStat::def, IV(0_bi), EV(0_bi));
...
void special_defense_tests() {
	...
	auto pokemon = defender.pokemon();
	pokemon.switch_in(generation, weather);
	set_stat_ev(pokemon, RegularStat::spd, IV(31_bi), EV(EV::max));
...
void speed_tests() {
	...
	pokemon.switch_in(generation, weather);
	set_stat_ev(pokemon, RegularStat::spe, IV(31_bi), EV(EV::max));
