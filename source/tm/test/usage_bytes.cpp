...
constexpr auto no_items(Generation const generation) {
	return generation >= Generation::two ?
		containers::dynamic_array<std::byte>(containers::concatenate_view(
			count_bytes(1),
			item_bytes(Item::None),
			weight_bytes(1.0)
		)) :
		containers::dynamic_array<std::byte>();
}
...
export constexpr auto smallest_team_bytes(Generation const generation) {
	return containers::dynamic_array<std::byte>(containers::concatenate_view(
