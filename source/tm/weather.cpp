...
export constexpr auto weather_is_blocked_by_ability(Ability const ability) {
	switch (ability) {
...
export constexpr auto weather_is_blocked_by_ability(Ability const ability1, Ability const ability2) {
	return weather_is_blocked_by_ability(ability1) or weather_is_blocked_by_ability(ability2);
}

export enum class NormalWeather : std::uint8_t {
	clear,
	hail,
	sand,
	sun,
...
export constexpr auto to_normal_weather(Weather const weather) -> NormalWeather {
	if (weather.hail()) {
