...
auto parse_html_team(DelimitedBufferView<std::string_view> buffer) -> SeenTeam<generation> {
	...
	for (auto const index : containers::integer_range(max_pokemon_per_team)) {
		...
		auto get_integer_wrapper = [&]<typename T>(bounded::type_t<T>, std::string_view const key) {
			return get_expected_integer_wrapper<T>(buffer.pop(), containers::concatenate<containers::string>(key, index_str));
		};
		auto get = [&]<typename T>(bounded::type_t<T>, std::string_view const key) {
		...
		auto species = get(bounded::type<Species>, "species");
		auto level = get_integer_wrapper(bounded::type<Level>, "level");
		auto gender = get(bounded::type<Gender>, "gender");
		auto item = get(bounded::type<Item>, "item");
		auto ability = get(bounded::type<Ability>, "ability");
		auto nature = get(bounded::type<Nature>, "nature");
