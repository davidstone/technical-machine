...
template<any_team TeamType>
auto execute_move(TeamType const & user, SelectedAndExecuted const move, TeamType const & other, OtherMove const other_move, Weather const weather, Depth const depth, auto const continuation) -> double {
...
struct Evaluator {
	...
	auto select_type_of_move(Team<generation> const & ai, LegalSelections const ai_selections, Team<generation> const & foe, LegalSelections const foe_selections, Weather const weather, Depth const depth, auto && ... respond_to_foe_moves_args) -> ScoredMoves {
		...
		m_transposition_table.add_score(ai, foe, weather, depth, moves);
		return moves;
	}

private:
	auto select_type_of_move(Team<generation> const & ai, Team<generation> const & foe, Weather const weather, Depth const depth) -> ScoredMoves {
	...
	auto use_move_branch_inner(KnownMove const first_used_move) {
		return [=, this](Team<generation> const & first, MoveName const first_move [[maybe_unused]], Team<generation> const & last, MoveName const last_move, Weather const weather, Depth const depth) {
			...
			return score_executed_moves(last, last_move, first, first_used_move, weather, depth, [&](Team<generation> const & updated_last, Team<generation> const & updated_first, Weather const updated_weather) {
	...
	auto use_move_branch_outer(OriginalPokemon const original_last_pokemon, MoveName const last_move) {
		return [=, this](Team<generation> const & first, MoveName const first_move, Team<generation> const & last, MoveName, Weather const weather, Depth const depth) {
			return score_executed_moves(first, first_move, last, FutureMove{is_damaging(last_move)}, weather, depth, [&](Team<generation> const & pre_updated_first, Team<generation> const & pre_updated_last, Weather const pre_updated_weather) {
				...
				return score_executed_moves(pre_updated_last, actual_last_move, pre_updated_first, first_used_move, pre_updated_weather, depth, [&](Team<generation> const & updated_last, Team<generation> const & updated_first, Weather const updated_weather) {
					...
					return max_score(m_respond_to_foe_moves(updated_first, first_selections, updated_last, last_selections, updated_weather, m_evaluate, depth, use_move_branch_inner(first_used_move)));
				});
			});
		};
	}
	...
	auto use_move_branch(Team<generation> const & first, MoveName const first_move, Team<generation> const & last, MoveName const last_move, Weather const weather, Depth const depth) -> double {
		...
		return score_executed_moves(first, first_move, last, FutureMove{is_damaging(last_move)}, weather, depth, [&](Team<generation> const & updated_first, Team<generation> const & updated_last, Weather const updated_weather) {
			...
			return max_score(m_respond_to_foe_moves(updated_first, first_selections, updated_last, last_selections, updated_weather, m_evaluate, depth, use_move_branch_outer(original_last_pokemon, last_move)));
		});
	}
	...
	auto end_of_turn_branch(Team<generation> first, Team<generation> last, Weather weather, Depth const depth, EndOfTurnFlags const first_flag, EndOfTurnFlags const last_flag) -> double {
		...
		if (first.pokemon().hp().current() == 0_bi or last.pokemon().hp().current() == 0_bi) {
			...
			return max_score(m_respond_to_foe_moves(first, first_selections, last, last_selections, weather, m_evaluate, depth, [&](auto && ... args) {
	...
	auto finish_end_of_turn(Team<generation> const & first, Team<generation> const & last, Weather const weather, Depth const depth) -> double {
		...
		if (new_depth.general > 0_bi) {
			return max_score(select_type_of_move(ai, foe, weather, new_depth));
	...
	auto evaluate_single_matchup(Team<generation> ai, TeamIndex const ai_index, Team<generation> foe, TeamIndex const foe_index, Weather weather, Depth const depth) -> double {
		...
		return max_score(select_type_of_move(ai, foe, weather, depth));
	}


	auto score_executed_moves(Team<generation> const & user, MoveName const selected_move, Team<generation> const & other, OtherMove const other_move, Weather const weather, Depth const depth, auto const continuation) const -> double {
		double score = 0.0;
		auto const executed_moves = possible_executed_moves(selected_move, user);
		for (auto const executed_move : executed_moves) {
			score += execute_move(user, SelectedAndExecuted{selected_move, executed_move}, other, other_move, weather, depth, continuation);
		}
